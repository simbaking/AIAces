...
public class GlobalAi {
    ...
    public static List<Double> feedForwardSafe(List<Double> inputs) {
        synchronized (INSTANCE) {
            return INSTANCE.feedForward(inputs);
        }
    }

    /** Thread-safe train — also queues sample to remote TF training server */
    public static void trainSafe(List<Double> inputs, int actionIndex, double reward) {
        synchronized (INSTANCE) {
            INSTANCE.train(inputs, actionIndex, reward, 1.0);
        }
        TrainingClient.getInstance().queueSample(inputs, actionIndex, reward);
    }

    /** Thread-safe train with multiplier — also queues sample to remote TF training server */
    public static void trainSafe(List<Double> inputs, int actionIndex, double reward, double weightMultiplier) {
        synchronized (INSTANCE) {
    ...
    /** Thread-safe mutate */
    public static void mutateSafe(double rate, double strength) {
        synchronized (INSTANCE) {
    ...
    public static void save() {
        synchronized (INSTANCE) {
            try {
                ...
                // Also trigger a git push of the TF weights so latest-ai branch stays current
                TrainingClient.getInstance().forcePush();
            } catch (IOException e) {
