 ...
 */
@Service
public class BackgroundTrainer implements CommandLineRunner {
    ...
    private void playOneGame() {
        ...
        while (!sim.isGameOver() && turnCount < MAX_TURNS_PER_GAME && running.get()) {
            ...
            // NOTE: We do NOT capture opponent distances here. The rolling-window reward
            // below is purely based on the CURRENT player's own progress and never penalizes
            // them for what opponents did to their stack between turns.
            ...
            // Track whether ANY card this turn moved the player farther from Ace —
            // if so, all bonus rewards for the entire turn are suppressed.
            boolean movedAwayThisTurn = false;
            ...
            while (keepStacking && !sim.isGameOver()) {
                ...
                // --- TEACHER SHADOWING ---
                int teacherAction = RuleBasedAi.getDecision(sim, current);
                if (teacherAction != -1) {
                    GlobalAi.trainSafe(freshInputs, teacherAction, 1.5); // High reward for imitation learning
                }
                // -------------------------
                        ...
                        if (handIdx != -1) {
                            ...
                            if (valid) {
                                ...
                                if (stackImprovement > 0) {
                                    // Reward for getting closer — suppress proximity bonus if any card this
                                    // turn already moved away from Ace (no compounding bonus on tainted turns).
                                    double stackReward = 10.0 * stackImprovement;
                                    if (!movedAwayThisTurn && distAfterStack <= 2) stackReward *= 2.0;
                                    GlobalAi.trainSafe(inputs, chosenActionIndex, stackReward);
                                } else if (stackImprovement == 0) {
                                    // No reward for placing a card that doesn't advance you toward Ace
                                    // Intentionally 0 — neutral placements get no signal
                                } else {
                                    // Heavy punishment for placing a card that takes you further from Ace
                                    movedAwayThisTurn = true; // Flag: suppress all bonuses for this turn
                                    double stackPenalty = -30.0;
                                    GlobalAi.trainSafe(inputs, chosenActionIndex, stackPenalty);
                                }
            ...
            // 4. Measure distance to Ace AFTER this turn
            double distNow = AiInputMapper.getDistanceToAce(current);
            ...
            //
            // IMPORTANT: If this player's distance is higher than the window average, it could
            // be because an opponent used an effect card (8-steal, 10-strip, 7-sabotage) to
            // damage this player's stack between turns. We NEVER penalize for negative rolling
            // progress — only reward genuine improvement. The signal is simply zeroed out.
            // Bonus rewards are also suppressed if any card THIS turn moved away from Ace.
            if (window.size() >= 2) {
                ...
                if (progress > 0 && !movedAwayThisTurn) {
                    // Only reward genuine net-progress turns; zero signal otherwise
                ...
                // progress <= 0 or movedAwayThisTurn: no reward, no penalty — zero signal
            }
    ...
    private int playOneSoloGame() {
        ...
        while (turnCount < MAX_TURNS_PER_GAME && running.get()) {
            ...
            while (keepStacking && !reachedAce) {
                ...
                // --- TEACHER SHADOWING ---
                int teacherAction = RuleBasedAi.getDecision(sim, solo);
                if (teacherAction != -1) {
                    GlobalAi.trainSafe(freshInputs, teacherAction, 1.5); // High reward for imitation learning
                }
                // -------------------------
                    ...
                    if (action >= 4) { // STACK Specific Card
                        ...
                        if (handIdx != -1) {
                            ...
                            if (valid) {
                                ...
                                double stackImprovement = distBefore - distAfterStack;
                                if (stackImprovement > 0) {
                                    // Reward for getting closer — suppress proximity bonus if any card
                                    // this turn already moved away from Ace.
                                    double stackReward = 10.0 * stackImprovement;
                                    if (!movedAwayThisTurn && distAfterStack <= 2) stackReward *= 2.0;
                                    GlobalAi.trainSafe(inputs, chosenActionIndex, stackReward);
                                } else if (stackImprovement == 0) {
            ...
            // Suppress all bonuses if any card this turn moved away from Ace.
            if (distanceWindow.size() >= 2 && !movedAwayThisTurn) {
                double sum = 0;
                ...
                if (progress > 0) {
                    // Only give reward on turns where no card moved away from Ace
                    double reward = Math.min(0.3 * progress, 3.0);
                    GlobalAi.trainSafe(inputs, chosenActionIndex, reward);
                }
                // progress == 0: no signal; progress < 0: no bonus (penalty already fired per-card above)
            }
    ...
    private void playStackFocusedGame() {
        ...
        while (turnCount < MAX_TURNS_PER_GAME && running.get()) {
            ...
            //    Track whether ANY card this turn moved away from Ace — if so, suppress all bonuses.
            ...
            while (keepStacking && !reachedAce) {
                ...
                for (int action : freshActions) {
                    if (action >= 4) { // STACK Specific Card
                        ...
                        if (improvement > 0) {
                            // Reward for getting closer — suppress proximity bonuses if any card
                            // this turn already moved away from Ace.
                            double stackReward = Math.min(15.0 * improvement, 50.0);
                            if (!movedAwayThisTurn) {
                            ...
                            // Very heavy punishment for placing a card that takes you further from Ace
                            movedAwayThisTurn = true; // Flag: suppress all bonuses for this turn
                            double stackPenalty = -40.0;
                            GlobalAi.trainSafe(inputs, chosenActionIndex, stackPenalty);
                        }
