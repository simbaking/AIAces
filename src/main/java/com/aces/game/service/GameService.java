...
public class GameService {
    ...
    /**
     * Tracks whether any card played during the current CPU multi-stack turn
     * moved the player farther from Ace. When true, proximity bonuses are
     * suppressed for all remaining cards on the same turn.
     * Reset to false at the start of each executeCpuStep() call.
     */
    private boolean turnMovedAwayFromAce = false;
    ...
    public void startGame() {
        ...
        if (defaultGame != null && defaultGame.isTrainingView()) {
            startTrainingView(count);
        } else {
            startGame(name, count);
        }
    }

    public void startTrainingView(int cpuCount) {
        defaultGame = new GameState();
        defaultGame.setInitialCpuCount(cpuCount);
        defaultGame.setPhase(GameState.Phase.PLAYING);
        defaultGame.setInitialPlayerName("Observer");
        defaultGame.setTrainingView(true);

        // Create Deck
        initializeDeck(defaultGame);

        // AI Observer (occupies human slot in UI so cards are face-up)
        Player p1 = new Player("p1", "AI Observer", true);
        defaultGame.getPlayers().add(p1);
        ...
        defaultGame.setGameMessage("Training View started. Auto-playing...");
        defaultGame.setCurrentPlayerIndex(0);
        defaultGame.setCpuTurnPending(true); // AI Observer plays first
    }
    ...
    public Card drawCardAndReturn(String playerId) {
        if (defaultGame.isGameOver()) return null;
        Player p = defaultGame.getCurrentPlayer();
        if (!p.getId().equals(playerId)) {
            return null;
        }
    ...
    public void playToStack(String playerId, int cardIndex) {
        ...
        if (isValid) {
            // Capture own distance BEFORE placing THIS card.
            // preDistSelf already reflects the current state of the stack (including any prior
            // damage from opponent effect cards like 8-steal or 10-strip). The improvement
            // measurement therefore only rewards/penalizes the CPU for its OWN card placement,
            // never for anything an opponent did on a previous turn.
            ...
            if (p.isPc() && preMoveInputs != null) {
                ...
                if (improvement > 0) {
                    ...
                    // Only apply proximity bonus if no card this turn has already moved away from Ace
                    if (!turnMovedAwayFromAce && postDist <= 2) reward *= 2.0;
                    com.aces.game.ai.GlobalAi.trainSafe(preMoveInputs, actionIndex, reward, 50.0);
                } else if (improvement < 0) {
                    // Scaled punishment: each step away costs -5.0, capped at -30.0
                    // This always exceeds the max positive reward (+3.0), discouraging bad moves strongly
                    turnMovedAwayFromAce = true; // Flag: suppress bonuses for rest of this turn
                    int actionIndex = 4 + com.aces.game.ai.AiInputMapper.getCardIndex(card);
                    double penalty = Math.max(improvement * 5.0, -30.0);
                    com.aces.game.ai.GlobalAi.trainSafe(preMoveInputs, actionIndex, penalty, 50.0);
                }
            }
            ...
            if (card.getRank() == Card.Rank.ACE) {
                ...
                defaultGame.setCpuTurnPending(false); // Prevent any further CPU steps
    ...
    private boolean isSequenceValid(Player p, Card top, Card card) {
        // Ace is terminal, no cards can be played on it
                        ...
                        if (defaultGame.isJokerModeToStack()) {
                            ...
                            if (found) {
                                ...
                                if (picked.getRank() == Card.Rank.ACE) {
                                    defaultGame.setGameOver(true);
                                    defaultGame.setCpuTurnPending(false); // Prevent ghost CPU steps
                                    // Start background training when game ends
                                    backgroundTrainer.start();
                                    defaultGame.setWinner(p);
    ...
    private void endTurn() {
        if (!defaultGame.isGameOver()) {
            ...
            com.aces.game.ai.GlobalAi.mutateSafe(0.1, 0.05);

            defaultGame.nextTurn();
            // If next player is CPU (or it's training view), set pending flag for animated step execution
            if (!defaultGame.getCurrentPlayer().isPc() || defaultGame.isTrainingView()) {
     ...
     * Proximity bonus is NEVER applied when the net movement was backward.
     *
    ...
    private double computeLiveReward(double preDistSelf, double postDistSelf) {
        ...
        if (progress > 0) {
            ...
            // Only apply proximity bonus on genuinely forward moves
            if (postDistSelf <= 2) reward *= 2.0;
            return reward;
        } else if (progress < 0) {
            // Scaled penalty: each step away costs -5.0, capped at -30.0
            // Always exceeds max positive reward (+3.0) to strongly discourage bad moves
            return Math.max(progress * 5.0, -30.0);
        }
        return 0.0; // No reward for standing still
    }
    ...
    public void processCpuStep(int delayMs) {
        ...
        if (current.isPc() && !defaultGame.isTrainingView()) // If human (and not training view), don't process
            return;
    ...
    private void executeCpuStep(Player cpu) {
        // Skip if called for human player (unless it's training view)
        if (cpu.isPc() && !defaultGame.isTrainingView()) {
            defaultGame.setCpuTurnPending(false);
            return;
        }
        ...
        // Reset the turn-level backward-move flag before starting a new CPU multi-stack turn
        turnMovedAwayFromAce = false;

        // 2. Brain Decision
        List<Double> inputs = com.aces.game.ai.AiInputMapper.extractInputs(defaultGame, cpu);
        List<Double> outputs = com.aces.game.ai.GlobalAi.feedForwardSafe(inputs);
        ...
        while (keepStacking && !defaultGame.isGameOver()) {
            ...
            final List<Double> freshOutputs = com.aces.game.ai.GlobalAi.feedForwardSafe(freshInputs);
            inputs = freshInputs;
                ...
                if (action >= 4) { // PLAY STACK Specific Card
                    ...
                    if (handIdx != -1) {
                        ...
                        if (valid) {
                            ...
                            acted = true;
                            // Check game-over IMMEDIATELY before any delay — Ace placement must register without lag
                            if (defaultGame.isGameOver()) return;
                    ...
                    if (!defaultGame.isHasPlayedToStack()) {
                        ...
                        if (idx != -1) {
                            ...
                            if (defaultGame.getEffectState() != GameState.EffectState.NONE) {
                                defaultGame.setCpuTurnPending(true);
                            } else if (!defaultGame.getCurrentPlayer().isPc() || defaultGame.isTrainingView()) {
                                defaultGame.setCpuTurnPending(true);
                            } else {
                                defaultGame.setCpuTurnPending(false);
                            }
