...
public class GameController {
    ...
    @PostMapping("/start-training-view")
    public String startTrainingView(@RequestParam(defaultValue = "1") int cpuCount) {
        gameService.startTrainingView(cpuCount);
        return "redirect:/game";
    }
    ...
    @PostMapping("/game/auto-restart")
    public String autoRestart() {
        GameState current = gameService.getGame();
        int cpuCount = (current != null) ? current.getInitialCpuCount() : 1;
        // Always restart as training view (this endpoint is only called from training view)
        gameService.startTrainingView(cpuCount);
        return "redirect:/game";
    }
